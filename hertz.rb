...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Dr. Roland Bernheimer - Cyan
# Karen Wagner - Magenta
# Dr. Isabelle Jung - Grün
# Lucas Lindner, Alois Hofer, Vicky Wilson, Antonio und Stella Bennini - Weiß
# alle anderen – Gelb
...
build_DFXL = lambda do |subFile|
  ...
  b.tt(:xmlns => "http://www.w3.org/2006/10/ttaf1",  'xmlns:tts' => "http://www.w3.org/2006/10/ttaf1#styling") do
    b.head do
      b.styling do
        b.style("xml:id"=>"default.left", "tts:fontFamily"=>"Arial", "tts:fontSize"=>"10px", "tts:textAlign"=>"left", "tts:fontStyle"=>"normal", "tts:fontWeight"=>"normal", "tts:backgroundColor"=>"transparent", "tts:color"=>"#FFFF00")
    ...
    b.body do
      b.div do
        subFile.dialogs.each do |dialog|
          ...
          when WHITE
            b.p(:begin =>"#{sprintf("%.2f", intime)}s", :end => "#{sprintf("%.2f", outtime)}", :dur => "#{sprintf("%.2f", dialog.duration)}s", :style => "default.center", "tts:direction" => "ltr") do
              b.span(dialog.text, 'tts:color'=>"#FFFFFF")
            end
          else
